...
void LeavePanel::OnShow(wxShowEvent &event)
{
    if (event.IsShown())
    {
        std::shared_ptr<LeaveBalance> leaveBalanceClass = dm->getLeaveBalance(dm->getCurrentEmployee()->getName());
        int casualLeaveBalance = leaveBalanceClass->getCasualLeave(wxDateTime::Now().GetYear());
        int earnedLeaveBalance = leaveBalanceClass->getEarnedLeave(wxDateTime::Now().GetYear());
        leaveBalance->SetLabelText("Leave Balance: Casual - " + std::to_string(casualLeaveBalance) + ", Earned - " + std::to_string(earnedLeaveBalance));
        UpdateUI();
    }
...
void LeavePanel::OnSubmit(wxCommandEvent &event)
{
    ...
    {
        if (leaveBalanceClass->applyCasualLeave(days, year) == false)
        {
        ...
        leaveBalance->SetLabelText("Leave Balance: Casual - " + std::to_string(leaveBalanceClass->getCasualLeave(wxDateTime::Now().GetYear())) + ", Earned - " + std::to_string(leaveBalanceClass->getEarnedLeave(wxDateTime::Now().GetYear())));
    }
    else if (leaveTypeValue == "Earned Leave")
    {
        if (leaveBalanceClass->applyEarnedLeave(days, year) == false)
        {
            wxMessageBox("Insufficient leave balance.", "Error", wxOK | wxICON_ERROR);
            return;
        }
